...
export class AStar{
    ...
    public *IterateUntilGoalBlockReached(goalNodePromiseResolve: (goalNode: IAStarNode) => void){
        ...
        while (openList.length > 0){
            ...
            const locationHash: string = Vector3Utils.toString(nextNode.Block.location);

            // Check if the nextNode is the EndBlock
            if (Vector3Utils.equals(nextNode.Block, this.EndBlock)){
            ...
            for (const location of surroundingLocations){
                ...
                if (blockAtLocation !== undefined && blockAtLocation.isValid){
                    ...
                    if (Vector3Utils.equals(location, this.Options.GoalLocation)){
            ...
            for (const surroundingBlock of surroundingBlocks){
                ...
                const surroundingBlockLocationHash: string = Vector3Utils.toString(surroundingBlock.location);

                
                // Check if this block is the end block
                if (Vector3Utils.equals(surroundingBlock, this.EndBlock)){
    ...
    private SetDebugBlock(block: Block) {
        ...
        if(!(Vector3Utils.toString(block.location) 
            in 
            [...this.TotalDebugBlocksCreated].map((b: Block)=> Vector3Utils.toString(b.location)))
    ...
    private ClearDebugBlocks() {
        ...
        for(const v of locs){
            system.run(() => {
                const vx = v.location;
                if(Vector3Utils.equals(vx, this.Options.StartLocation) || Vector3Utils.equals(vx, this.Options.GoalLocation)) return
    ...
    private GetIndexOfNodeIfInList(node: IAStarNode, listOfNodes: IAStarNode[]): number | null{
        for (const index in listOfNodes){
            ...
            if (Vector3Utils.equals(node.Block, nodeInList.Block)){
                return indexNumber;
            }
        }
